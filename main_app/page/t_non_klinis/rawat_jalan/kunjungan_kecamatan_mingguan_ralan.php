...
require_once __DIR__ . '/kunjungan_kecamatan_mingguan_ralan_helper.php';
...
$report = aptd_kec_mingguan_fetch($conn, $startDate, $endDate, $weeks);
$rows = $report['rows'];
...
$exportExcelAction = 'page/t_non_klinis/rawat_jalan/export_kunjungan_kecamatan_mingguan_ralan_excel.php';
$exportPdfAction = 'page/t_non_klinis/rawat_jalan/export_kunjungan_kecamatan_mingguan_ralan_pdf.php';
?>
<br>
<style>
...
<div class="kecm-wrap">
    <section class="kecm-panel">
        <h1 class="kecm-title">Data Kunjungan Pasien Rawat Jalan per Kecamatan per Minggu</h1>
        <p class="kecm-sub">Periode minggu mengikuti Selasa sampai Senin dan dibatasi di dalam bulan yang dipilih.</p>
        <form id="filterKecMingguanForm" method="post" class="kecm-filter">
            ...
            <?php if ($isAdmin): ?>
                <button type="button" class="btn btn-success btn-sm px-4" id="btnExportKecMingguanExcel">Export Excel</button>
            <?php endif; ?>
            <button type="button" class="btn btn-danger btn-sm px-4" id="btnExportKecMingguanPdf">Export PDF</button>
        </form>
    </section>
    ...
    <section class="kecm-panel">
        <div class="kecm-head">
            <div>
                <h2 class="kecm-title" style="font-size:18px;">Rekap Kecamatan per Minggu</h2>
                <p class="kecm-sub">Total kunjungan sesuai mapping kecamatan: <?php echo number_format($totals['total'], 0, ',', '.'); ?> pasien.</p>
            </div>
...
<script>
(function(){
    var form = document.getElementById('filterKecMingguanForm');
    if (!form) return;
    ...
    var excelButton = document.getElementById('btnExportKecMingguanExcel');
    if (excelButton) {
    ...
    var pdfButton = document.getElementById('btnExportKecMingguanPdf');
    if (pdfButton) {
