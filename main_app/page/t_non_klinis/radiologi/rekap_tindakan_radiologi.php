...
function rekapRadiologiValidMonth($value)
{
    $month = (int) $value;
    return ($month >= 1 && $month <= 12) ? $month : (int) date('n');
}

function rekapRadiologiValidYear($value)
{
    $year = (int) $value;
    return ($year >= 2000 && $year <= 2100) ? $year : (int) date('Y');
}

$filterMonth = rekapRadiologiValidMonth(isset($_POST['bulan']) ? $_POST['bulan'] : date('n'));
$filterYear = rekapRadiologiValidYear(isset($_POST['tahun']) ? $_POST['tahun'] : date('Y'));
$monthNames = [
    1 => 'Januari',
    2 => 'Februari',
    3 => 'Maret',
...
$sql = "SELECT
            COALESCE(NULLIF(TRIM(jpr.nm_perawatan), ''), pr.kd_jenis_prw) AS pemeriksaan_radiologi,
        ...
        WHERE pr.tgl_periksa >= ?
          AND pr.tgl_periksa < ?
        GROUP BY pemeriksaan_radiologi
        ORDER BY jumlah_pemeriksaan DESC, pemeriksaan_radiologi ASC";
...
<div class="row text-left">
    <div class="col">
        <h3 class="text-left" style="color: #666666; margin-bottom: 5px;">REKAPITULASI JUMLAH TINDAKAN PEMERIKSAAN RADIOLOGI</h3>
            ...
            <div class="wrapper">
                ...
                <table class="table table-sm table-bordered table-hover" id="table4" style="width:100%; margin-top: 10px; font-size: 12px;">
                    ...
                    <tbody>
                        <?php if (empty($rows)): ?>
                            <tr>
                                <td colspan="3" style="text-align: center;">Tidak ada data/pemeriksaan radiologi pada periode ini</td>
                            </tr>
                        <?php else: ?>
                            <?php $no = 1; foreach ($rows as $row): ?>
                                <tr>
                                    <td style="text-align: center;"><?php echo $no++; ?></td>
                                    <td><?php echo htmlspecialchars($row['pemeriksaan_radiologi'], ENT_QUOTES, 'UTF-8'); ?></td>
