...
if (!aptd_can_access($level, 'export_evaluasi_task4_terawal_bpjs')) {
...
try {
    $filters['search'] = '';
    $filters['page'] = 1;
    $report = aptd_task4_terawal_build_report($mysqli, $filters, false);
    $rows = [];
    ...
    list($spreadsheet, $sheet) = aptd_excel_create(
        'Indikator Kesesuaian Jadwal Dokter - Task ID 4 Terawal',
    ...
    $filename = 'indikator-jadwal-task-4-terawal-' . $filters['tanggal_awal'] . '-sampai-' . $filters['tanggal_akhir'] . '.xlsx';
    aptd_excel_output($spreadsheet, $filename);
} catch (Throwable $exception) {
    error_log('Export Evaluasi Task ID 4 Terawal: ' . $exception->getMessage());
