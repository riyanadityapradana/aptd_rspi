...
$tglAwal = isset($_POST['tgl_awal']) && $_POST['tgl_awal'] !== '' ? $_POST['tgl_awal'] : date('Y-m-01');
$tglAkhir = isset($_POST['tgl_akhir']) && $_POST['tgl_akhir'] !== '' ? $_POST['tgl_akhir'] : date('Y-m-d');
...
$sql = "SELECT p.nm_penyakit, COUNT(*) AS jumlah_kasus
        ...
        WHERE r.status_lanjut = 'Ralan'
          ...
          AND DATE(r.tgl_registrasi) BETWEEN ? AND ?
...
<div class="row">
    <div class="col-md-6">
        <div class="table-responsive-sm"><table class="table table-sm table-bordered table-hover" id="table4" style="width:100%;font-size:12px;"><thead class="thead-dark"><tr><th>No</th><th>Nama Penyakit</th><th>Jumlah</th><th>%</th></tr></thead><tbody><?php if (empty($rows)): ?><tr><td colspan="4" style="text-align:center;">Tidak ada data penyakit pada periode ini.</td></tr><?php else: $no = 1; foreach ($rows as $row): $persen = $totalKasus > 0 ? ((int) $row['jumlah_kasus'] / $totalKasus) * 100 : 0; ?><tr><td style="text-align:center;"><?php echo $no++; ?></td><td><?php echo htmlspecialchars($row['nm_penyakit'], ENT_QUOTES, 'UTF-8'); ?></td><td style="text-align:center;font-weight:bold;"><?php echo number_format((int) $row['jumlah_kasus'], 0, ',', '.'); ?></td><td style="text-align:center;"><?php echo number_format($persen, 1, ',', '.'); ?>%</td></tr><?php endforeach; endif; ?></tbody></table></div>
    </div>
